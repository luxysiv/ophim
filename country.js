$(document).ready(function () {
    ...
    function renderPagination(currentPage, totalPages, slug) {
        ...
        if (currentPage > 1) {
            paginationHtml += `<li class="page-item"><a class="page-link" href="country.html?slug=${slug}&page=${currentPage - 1}"><i class="fas fa-angle-left"></i></a></li>`;
        }
        ...
        for (let i = startPage; i <= endPage; i++) {
            ...
            paginationHtml += `
                <li class="page-item ${active}">
                    <a class="page-link" href="country.html?slug=${slug}&page=${i}">${i}</a>
                </li>
            `;
        }
        
        if (currentPage < totalPages) {
             paginationHtml += `<li class="page-item"><a class="page-link" href="country.html?slug=${slug}&page=${currentPage + 1}"><i class="fas fa-angle-right"></i></a></li>`;
        }
    ...
    function loadCountryMovies(countrySlug, page = 1) {
        const apiUrl = `${API_BASE_URL}quoc-gia/${countrySlug}?page=${page}`;
        $.getJSON(apiUrl, function (data) {
            if (data.status === 'success') {
                renderMovies(data.data.items);
                renderPagination(data.data.pagination.currentPage, data.data.pagination.totalPages, countrySlug);
                $('#page-title').text(`Phim Quốc Gia: ${data.data.titlePage} 🌎`);
                $('title').text(`Phim Quốc Gia: ${data.data.titlePage}`);
            }
        });
    }
    ...
    const countrySlug = getUrlParameter('slug');
    const page = parseInt(getUrlParameter('page')) || 1;

    if (countrySlug) {
        loadCountryMovies(countrySlug, page);
    } else {
        $('#movie-list').html('<div class="col-12"><p class="text-center text-muted">Vui lòng chọn một quốc gia từ menu.</p></div>');
    }
